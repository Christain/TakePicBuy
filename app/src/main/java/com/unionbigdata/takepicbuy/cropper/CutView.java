...
public class CutView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (m_bmp == null && getWidth() > 0 && getHeight() > 0) {
            ...
            if (m_bmp != null) {
                m_bmp = MakeBmp.CreateBitmap(m_bmp, getWidth(), getHeight(),
                        -1, 0, Config.ARGB_8888);
        ...
        if (m_bmp != null) {
            if (m_matrix == null) {
                ...
                m_matrix.postScale(m_sx, m_sy, m_x + m_bmp.getWidth() / 2, m_y
                        + m_bmp.getHeight() / 2); // 缩放、倒置
                m_matrix.postRotate(m_r, m_x + m_bmp.getWidth() / 2, m_y
                        + m_bmp.getHeight() / 2);
            }
            ...
            if (isDisplay) {
                ...
                canvas.drawRect(0, 0, getWidth(), (int) temp_dst2[1],
                        temp_paint); // 上
                canvas.drawRect(0, (int) temp_dst2[1], (int) temp_dst2[0],
                        (int) temp_dst2[3], temp_paint); // 左
                canvas.drawRect((int) temp_dst2[2], (int) temp_dst2[1],
                        getWidth(), (int) temp_dst2[3], temp_paint); // 右
                canvas.drawRect(0, (int) temp_dst2[3], getWidth(), getHeight(),
                        temp_paint); // 下

                // 矩形
                temp_paint.setColor(0xFFFFFFFF);
                temp_paint.setStrokeWidth(8);
                canvas.drawLine(temp_dst2[0], temp_dst2[1], temp_dst2[2],
                        temp_dst2[1], temp_paint);
                canvas.drawLine(temp_dst2[2], temp_dst2[1], temp_dst2[2],
                        temp_dst2[3], temp_paint);
                canvas.drawLine(temp_dst2[2], temp_dst2[3], temp_dst2[0],
                        temp_dst2[3], temp_paint);
                canvas.drawLine(temp_dst2[0], temp_dst2[3], temp_dst2[0],
                        temp_dst2[1], temp_paint);
                ...
                canvas.drawLine(
                        (int) (temp_dst2[0] + (temp_dst2[2] - temp_dst2[0]) / 3),
                        (int) (temp_dst2[1]),
                        (int) (temp_dst2[0] + (temp_dst2[2] - temp_dst2[0]) / 3),
                        (int) (temp_dst2[3]), temp_paint);
                canvas.drawLine(
                        (int) (temp_dst2[0] + (temp_dst2[2] - temp_dst2[0]) * 2 / 3),
                        (int) (temp_dst2[1]),
                        (int) (temp_dst2[0] + (temp_dst2[2] - temp_dst2[0]) * 2 / 3),
                        (int) (temp_dst2[3]), temp_paint);
                canvas.drawLine(
                        (int) (temp_dst2[0]),
                        (int) (temp_dst2[1] + (temp_dst2[3] - temp_dst2[1]) / 3),
                        (int) (temp_dst2[2]),
                        (int) (temp_dst2[1] + (temp_dst2[3] - temp_dst2[1]) / 3),
                        temp_paint);
                canvas.drawLine(
                        (int) (temp_dst2[0]),
                        (int) (temp_dst2[1] + (temp_dst2[3] - temp_dst2[1]) * 2 / 3),
                        (int) (temp_dst2[2]),
                        (int) (temp_dst2[1] + (temp_dst2[3] - temp_dst2[1]) * 2 / 3),
                        temp_paint);

                temp_paint.setTextAlign(Align.CENTER);
                temp_paint.setTextSize(27);
                canvas.drawText(
                        (int) (temp_dst2[2] - temp_dst2[0]) + "*"
                                + (int) (temp_dst2[3] - temp_dst2[1]),
                        (int) ((temp_dst2[2] - temp_dst2[0]) / 2 + temp_dst2[0]),
                        (int) ((temp_dst2[3] - temp_dst2[1]) / 2 + temp_dst2[1]),
                        temp_paint);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mode == MOVE) {
                    ...
                    if (m_rect.m_x + m_rect.m_w >= 1f)
                        m_rect.m_x = 1f - m_rect.m_w;
                    if (m_rect.m_y + m_rect.m_h >= 1f)
                        m_rect.m_y = 1f - m_rect.m_h;
            ...
            case MotionEvent.ACTION_POINTER_DOWN:
                mode = DOUBLE_POINT_ZOOM;
                m_event = DOUBLE_POINT_DOWN;
                invalidate();
                break;
    ...
    public void setScale(float s) // 设置矩形的宽高比
    {
        ...
        isDisplay = true;
        if (m_scale != FREESCALE) {
            m_rect.m_h = ((m_rect.m_w * m_bmp.getWidth()) / m_scale)
                    / m_bmp.getHeight();
    ...
    private boolean isDwon(float x, float y, float dx, float dy) {
        if (dx >= x - RADIO && dx <= x + RADIO && dy >= y - RADIO
                && dy <= y + RADIO) {
            return true;
        } else {
            return false;
